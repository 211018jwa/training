...
public class StudentService {
	...
	public Student addStudent(AddOrUpdateStudentDTO dto) throws InvalidParameterException, SQLException {
		...
		if (dto.getFirstName().trim().equals("")) {
			e.messages.add("First name cannot be blank");
		}
		
		if (dto.getLastName().trim().equals("")) {
			e.messages.add("Last name cannot be blank");
		}
		...
		if (!validClassifications.contains(dto.getClassification())) {
			e.messages.add("You entered an invalid classification");
		}
		
		if (dto.getAge() < 0) {
			e.messages.add("Age cannot be less than 0");
		}
		
		// Check to see if the exception object HAS any messages (meaning that at least 1 thing was incorrect)
